...
function initial_setting_menu() {
  ...
  add_action( 'admin_init', 'register_themes_setting','admin-head');
}


function register_themes_setting() {
  register_setting( 'themes-initialize-group', 'blogname' );
  register_setting( 'themes-initialize-group', 'blogdescription' );
  register_setting( 'themes-initialize-group', 'meta_keywords' );
  register_setting( 'themes-initialize-group', 'banner_url' );
  register_setting( 'themes-initialize-group', 'banner_image' );
  // register_setting( 'themes-initialize-group', 'blog_public' );
  // register_setting( 'themes-initialize-group', 'permalink_structure' );

  //配色
  register_setting( 'themes-initialize-group', 'color_scheme');

  //トップページのロゴ設定
  register_setting( 'themes-initialize-group', 'toppage_logo_type' );
  register_setting( 'themes-initialize-group', 'logo_text' );
  register_setting( 'themes-initialize-group', 'logo_image' );

  //お問い合わせ先の設定
  register_setting( 'themes-initialize-group', 'cta_link' );

  //トップページのメタタグの設定
  //コアから取得

  //Googleツールの設定
  register_setting( 'themes-initialize-group', 'analytics_tracking_code' );

  // register_setting( 'themes-initialize-group', 'google_publisher' );

  //Facebookとの連携
  register_setting( 'themes-initialize-group', 'facebook_user_id' );
  register_setting( 'themes-initialize-group', 'facebook_app_id' );
  register_setting( 'themes-initialize-group', 'facebook_page_url' );

  //その他の設定
  register_setting( 'themes-initialize-group', 'def_image' );
  // register_setting( 'themes-initialize-group', 'post_layout' );
  // register_setting( 'themes-initialize-group', 'show_social_buttons' );
  // register_setting( 'themes-initialize-group', 'show_like_button' );
  // register_setting( 'themes-initialize-group', 'show_tweet_button' );
  // register_setting( 'themes-initialize-group', 'show_google_button' );
  // register_setting( 'themes-initialize-group', 'show_hatena_button' );
  // register_setting( 'themes-initialize-group', 'show_pocket_button' );
  // register_setting( 'themes-initialize-group', 'show_line_button' );

  register_setting( 'themes-initialize-group', 'twitter_id' );
  //.htaccessを更新させる必要がある
...
function banner_options_page() {
  ...
  <div class="wrap">
    ...
    <form method="post" action="options.php" enctype="multipart/form-data" encoding="multipart/form-data">
      ...
      settings_fields( 'themes-initialize-group' );
      do_settings_sections( 'themes-initialize-group' );
      ?>
      ...
      <div class="metabox-holder">
        ...
        <div class="metabox-holder">
          <div id="toppage_meta_setting" class="postbox " >
            <h3 class='hndle'><span>お問い合わせ先の設定</span></h3>
            <div class="inside">
              <div class="main">
                <p class="setting_description">ここではお問い合わせ先の設定を行います。</p>

                <h4>お問い合わせ先のリンク</h4>
                <p><input type="text" id="cta_link" class="regular-text" name="cta_link" value="<?php echo get_option('cta_link'); ?>"></p>
                <p class="setting_description"><small>お問い合わせフォーム、お問い合わせ先が記載されているページへのリンクを入力して下さい。<br>記入例：https://xxxxxxx.com/contact</small></p>
            ...
            <div class="inside">
              <div class="main">
                <p class="setting_description">Facebookとの連携を行います。これらを入力していなければ、LikeBOXの表示ができなかったり、シェアされた際の拡散効率が落ちてしまいますので、しっかりと設定しておきましょう。<br>
                  （※ 詳しくは<a href="https://themes.jp/themes-1st-settings" target="_blank">themesインストール後の初期設定</a>の「３．ソーシャルメディアのリンク設定」と「４．OGPの設定」をご確認ください）</p>
...
// 投稿画面の項目を非表示にする
function default_post_screen_metaboxes() {
...
add_action('admin_menu','default_post_screen_metaboxes');
